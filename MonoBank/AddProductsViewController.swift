...
class AddProductsViewController: UITableViewController, UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
	...
	override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		if indexPath.section == 2 && indexPath.row == 0 {
			...
			let mainViewController = navigationController?.viewControllers.first as! MarketViewController
			mainViewController.productsCollectionView.reloadData()
			mainViewController.productsNumberChanged()
